...
const theatreListings = require('./theatreinfo.json');
...
app.get('/', (req, res) => {
    res.send('This is bens EPIC api server! Please enter the extension of the API you would like to reach after the beep');
});

app.get('hellofuggomayihaveyourtheatrespliasixd', (req, res) => {
    res.send(theatreListings);
}
)
...
// For OMEGLE goes here. socketio connections
io.on("connection", (socket) => {
    ...
    socket.on("callUser", ({ userToCall, signalData, from, name }) => {
        console.log(name + " with the ID " + from + " is calling the ID " + userToCall );
        io.to(userToCall).emit("callUser", { signal: signalData, from, name });
    });
    socket.on("answerCall", (data) => {
        console.log("the data from this connected call is : " + data.signal)
        ...
        if (checkPost.isReply){
                ...
                if (highScores[i].postNumber === Number(req.body.motherPost)) {
                    for (let j =0; j < highScores[i].postReplies.length; j++) {
                        if (highScores[i].postReplies[j].postNumber === Number(req.body.postNumber)){
                            ...
                            let checkUserIP = String(req.socket.remoteAddress);
                            if (checkUserIP === checkReplyIP || checkUserIP === "::ffff:209.226.150.187") {
            ...
            for (let i = 0; i < highScores.length; i++) {
                if (highScores[i].postNumber === Number(req.body.postNumber)) {
                    ...
                    if (req.socket.remoteAddress === "209.226.150.187") {
                        let checkReplyIP = String(highScores[i].ip);
                        let checkUserIP = String(req.req.socket.remoteAddress);
                        if (checkUserIP === checkReplyIP || checkUserIP === "::ffff:209.226.150.187") {
...
server.listen(PORT, () => console.log(`fuggo number one SERVER is is is is runmning on port nimber # ${PORT}`));